// Package backend defines the contract every den storage engine
// implements: the [Backend] interface, plus the supporting types passed
// across it ([Query], [LockMode], [AggregateOp], etc.). Concrete backend
// implementations live in subpackages such as den/backend/sqlite and
// den/backend/postgres; they register themselves at init time via
// [github.com/oliverandrich/den.RegisterBackend].
//
// Most application code does not import this package directly — it works
// with backends through den's QuerySet and CRUD entry points. Direct
// imports are useful when building a custom backend, admin tooling, or
// migration logic that needs to dispatch on backend capabilities.
package backend
