...
func conBenchDBPostgres(b *testing.B) *engine.DB {
	b.Helper()
	return dentest.MustOpenPostgres(b, dentest.PostgresURL(), &BenchArticle{}, &BenchAuthor{}, &BenchJob{})
}

func conBenchDBSQLite(b *testing.B) *engine.DB {
	b.Helper()
...
func runConFindByID(b *testing.B, db *engine.DB) {
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			idx := counter.Add(1)
			if _, err := engine.FindByID[BenchArticle](ctx, db, ids[int(idx)%len(ids)]); err != nil {
				b.Fatal(err)
			}
		}
	})
...
func runConInsert(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
...
func runConMixed(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	ids := seedArticles(b, db, 1000, authorID)
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			i := counter.Add(1)
			if i%5 == 0 {
				doc, err := engine.FindByID[BenchArticle](ctx, db, ids[int(i)%len(ids)])
				if err != nil {
					b.Fatal(err)
				}
				doc.Stock++
				...
				if _, err := engine.FindByID[BenchArticle](ctx, db, ids[int(i)%len(ids)]); err != nil {
					b.Fatal(err)
				}
			}
		}
...
func runConQueueConsumer(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	...
	if err := engine.SaveAll(ctx, db, jobs); err != nil {
		b.Fatal(err)
	}
	...
	b.RunParallel(func(pb *testing.PB) {
		for pb.Next() {
			...
			_ = engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
				job, err := engine.LockByID[BenchJob](ctx, tx, ids[idx], engine.SkipLocked())
				if err != nil {
