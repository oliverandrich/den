...
type BenchArticle struct {
	...
	Author      engine.Link[BenchAuthor] `json:"author"`
...
func makeBenchArticle(i int, authorID string) *BenchArticle {
	return &BenchArticle{
		...
		Author:      engine.Link[BenchAuthor]{ID: authorID},
		Meta: map[string]any{
			"views":    i * 17,
			"featured": i%7 == 0,
			"locale":   "en-US",
...
func rwBenchDB(b *testing.B) *engine.DB {
	b.Helper()
	return dentest.MustOpen(b, &BenchArticle{}, &BenchAuthor{})
}

func rwBenchDBPostgres(b *testing.B) *engine.DB {
	b.Helper()
	return dentest.MustOpenPostgres(b, dentest.PostgresURL(), &BenchArticle{}, &BenchAuthor{})
}

func seedAuthor(b *testing.B, db *engine.DB) string {
	b.Helper()
	a := &BenchAuthor{Name: "Jane Author", Email: "jane@example.com", Bio: "Prolific writer."}
	if err := engine.Save(context.Background(), db, a); err != nil {
		b.Fatal(err)
	}
	return a.ID
}

func seedArticles(b *testing.B, db *engine.DB, n int, authorID string) []string {
	b.Helper()
	...
	if err := engine.SaveAll(ctx, db, docs); err != nil {
		b.Fatal(err)
	}
	ids := make([]string, n)
	for i, d := range docs {
...
func runRWInsert(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	...
	for b.Loop() {
		...
		if err := engine.Save(ctx, db, doc); err != nil {
			b.Fatal(err)
		}
		i++
	}
...
func BenchmarkRW_SQLite_Insert_Validated(b *testing.B) {
	...
	i := 0
	for b.Loop() {
		if err := engine.Save(ctx, db, makeBenchValidatedArticle(i, authorID)); err != nil {
			b.Fatal(err)
		}
		i++
	}
...
func runRWInsertMany(b *testing.B, db *engine.DB, batch int) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	b.ResetTimer()
	b.ReportAllocs()
...
func runRWFindByID(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	...
	i := 0
	for b.Loop() {
		if _, err := engine.FindByID[BenchArticle](ctx, db, ids[i%len(ids)]); err != nil {
			b.Fatal(err)
		}
		i++
	}
...
func runRWFindByIDs(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	ids := seedArticles(b, db, 1000, authorID)
	b.ResetTimer()
	...
	for b.Loop() {
		...
		if _, err := engine.FindByIDs[BenchArticle](ctx, db, batch); err != nil {
			b.Fatal(err)
		}
		i++
	}
...
func runRWQueryFiltered(b *testing.B, db *engine.DB, limit int) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	...
	b.ReportAllocs()
	for b.Loop() {
		results, err := engine.NewQuery[BenchArticle](db,
			where.Field("status").Eq("published"),
		).Sort("published_at", engine.Desc).Limit(limit).All(ctx)
		if err != nil {
			b.Fatal(err)
		}
		if len(results) == 0 {
...
func runRWIter(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	_ = seedArticles(b, db, 1000, authorID)
	b.ResetTimer()
	...
	for b.Loop() {
		count := 0
		for _, err := range engine.NewQuery[BenchArticle](db).Iter(ctx) {
			if err != nil {
				b.Fatal(err)
			}
			count++
...
func runRWCount(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	_ = seedArticles(b, db, 1000, authorID)
	b.ResetTimer()
	b.ReportAllocs()
	for b.Loop() {
		if _, err := engine.NewQuery[BenchArticle](db,
			where.Field("status").Eq("published"),
		).Count(ctx); err != nil {
			b.Fatal(err)
		}
...
func runRWSum(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	_ = seedArticles(b, db, 1000, authorID)
	b.ResetTimer()
...
func runRWSearch(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	_ = seedArticles(b, db, 1000, authorID)
	b.ResetTimer()
	b.ReportAllocs()
	for b.Loop() {
		results, err := engine.NewQuery[BenchArticle](db).Limit(20).Search(ctx, "Cicero")
		if err != nil {
			b.Fatal(err)
		}
		if len(results) == 0 {
...
func runRWWithFetchLinks(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
...
func runRWUpdate(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	...
	i := 0
	for b.Loop() {
		doc, err := engine.FindByID[BenchArticle](ctx, db, ids[i%len(ids)])
		if err != nil {
			b.Fatal(err)
		}
		doc.Stock++
...
func runRWBulkUpdate(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	_ = seedArticles(b, db, 100, authorID)
	b.ResetTimer()
	...
	for b.Loop() {
		...
		if _, err := engine.NewQuery[BenchArticle](db).Update(ctx, engine.SetFields{"status": newStatus}); err != nil {
			b.Fatal(err)
		}
		i++
	}
...
func runRWTransaction(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	authorID := seedAuthor(b, db)
	ids := seedArticles(b, db, 100, authorID)
	b.ResetTimer()
	...
	for b.Loop() {
		err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
			doc, err := engine.FindByID[BenchArticle](ctx, tx, ids[i%len(ids)])
			if err != nil {
				return err
			}
			doc.Stock++
			return engine.Save(ctx, tx, doc)
		})
		if err != nil {
			b.Fatal(err)
		}
