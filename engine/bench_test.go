...
func benchDBPostgres(b *testing.B) *engine.DB {
	b.Helper()
	return dentest.MustOpenPostgres(b, dentest.PostgresURL(), &BenchProduct{})
}

func seedBenchProducts(b *testing.B, db *engine.DB, n int) []string {
	b.Helper()
	...
	for i := range n {
		...
		if err := engine.Save(ctx, db, p); err != nil {
			b.Fatal(err)
		}
		ids[i] = p.ID
	}
...
func runInsertBenchmark(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	b.ResetTimer()
	b.ReportAllocs()
	for b.Loop() {
...
func runFindByIDBenchmark(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	...
	b.ReportAllocs()
	for b.Loop() {
		if _, err := engine.FindByID[BenchProduct](ctx, db, target); err != nil {
			b.Fatal(err)
		}
	}
}

func runQueryAllBenchmark(b *testing.B, db *engine.DB, n int) {
	ctx := context.Background()
	...
	b.ReportAllocs()
	for b.Loop() {
		results, err := engine.NewQuery[BenchProduct](db).All(ctx)
		if err != nil {
			b.Fatal(err)
		}
		if len(results) != n {
...
func runQueryIterBenchmark(b *testing.B, db *engine.DB, n int) {
	ctx := context.Background()
	seedBenchProducts(b, db, n)
	...
	for b.Loop() {
		count := 0
		for _, err := range engine.NewQuery[BenchProduct](db).Iter(ctx) {
			if err != nil {
				b.Fatal(err)
			}
			count++
...
func runUpdateBenchmark(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	p := &BenchProduct{Name: "Widget", Price: 9.99, Category: "test"}
	if err := engine.Save(ctx, db, p); err != nil {
		b.Fatal(err)
...
func runDeleteBenchmark(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	...
	for b.Loop() {
		...
		if err := engine.Delete(ctx, db, p); err != nil {
			b.Fatal(err)
		}
	}
}

func runQueryWithConditionBenchmark(b *testing.B, db *engine.DB) {
	ctx := context.Background()
	...
	b.ReportAllocs()
	for b.Loop() {
		results, err := engine.NewQuery[BenchProduct](db,
			where.Field("price").Gt(50.0),
		).Sort("price", engine.Asc).Limit(10).All(ctx)
		if err != nil {
			b.Fatal(err)
		}
		_ = results
