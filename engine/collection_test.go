...
func TestMeta(t *testing.T) {
	...
	// Find title field
	var titleField *engine.FieldMeta
...
func TestMeta_Flags_SoftDeleteOnly(t *testing.T) {
	...
	meta, err := engine.Meta[SoftProduct](db)
	require.NoError(t, err)
...
func TestMeta_Flags_RevisionOnly(t *testing.T) {
	...
	meta, err := engine.Meta[RevProduct](db)
	require.NoError(t, err)
	assert.False(t, meta.HasSoftDelete)
...
func TestMeta_Flags_BothSoftDeleteAndRevision(t *testing.T) {
	...
	meta, err := engine.Meta[SoftRevProduct](db)
	require.NoError(t, err)
	assert.True(t, meta.HasSoftDelete)
	assert.True(t, meta.HasRevision)
}
...
func TestMeta_Flags_ChangeTrackingOnly(t *testing.T) {
	...
	meta, err := engine.Meta[TrackedProduct](db)
	require.NoError(t, err)
	assert.False(t, meta.HasSoftDelete)
	assert.False(t, meta.HasRevision)
...
func TestMeta_Flags_AllThree(t *testing.T) {
	...
	meta, err := engine.Meta[AuditDoc](db)
	require.NoError(t, err)
	assert.True(t, meta.HasSoftDelete)
	assert.True(t, meta.HasRevision)
	assert.True(t, meta.HasChangeTracking)
...
func (AuditDoc) DenSettings() engine.Settings {
...
func TestMeta_Unregistered(t *testing.T) {
	...
	_, err := engine.Meta[Note](db)
	assert.ErrorIs(t, err, engine.ErrNotRegistered)
}
...
func TestErrNotRegistered_MessageIsActionable(t *testing.T) {
	...
	msg := err.Error()
	assert.Contains(t, msg, "engine_test.Note",
...
func (d CustomNameDoc) DenSettings() engine.Settings {
	return engine.Settings{CollectionName: "custom_docs"}
}
...
func TestRegister_RejectsInjectionInFieldName_SQLite(t *testing.T) {
	...
	for _, tt := range cases {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := engine.Register(context.Background(), db, tt.doc)
...
func TestRegister_CustomCollectionName(t *testing.T) {
	...
	meta, err := engine.Meta[CustomNameDoc](db)
	require.NoError(t, err)
	...
	found, err := engine.FindByID[CustomNameDoc](ctx, db, doc.ID)
	require.NoError(t, err)
...
func (d SettingsIndexDoc) DenSettings() engine.Settings {
	return engine.Settings{
		Indexes: []engine.IndexDefinition{{
...
func TestRegister_CompositeUniqueIndex(t *testing.T) {
	...
	meta, err := engine.Meta[CompositeUniqueDoc](db)
	require.NoError(t, err)

	// Should have one composite unique index
	var found *engine.IndexDefinition
...
func TestRegister_SettingsIndexes(t *testing.T) {
	...
	meta, err := engine.Meta[SettingsIndexDoc](db)
	require.NoError(t, err)
...
func (d *PtrSettingsDoc) DenSettings() engine.Settings {
	return engine.Settings{CollectionName: "ptr_settings_custom"}
}
...
func TestRegister_ValueWithPointerReceiverSettings(t *testing.T) {
	...
	require.NoError(t, engine.Register(ctx, db, PtrSettingsDoc{}))

	meta, err := engine.Meta[PtrSettingsDoc](db)
	require.NoError(t, err)
...
func TestOpenURL_WithTypes(t *testing.T) {
	...
	db, err := engine.OpenURL(ctx, dsn, engine.WithTypes(&Product{}, &Note{}))
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })

	cols := engine.Collections(db)
	assert.Contains(t, cols, "product")
	assert.Contains(t, cols, "note")

	require.NoError(t, engine.Save(ctx, db, &Product{Name: "W", Price: 1.0}))
}

func TestOpenURL_WithTypes_PropagatesRegistrationError(t *testing.T) {
	dsn := "sqlite:///" + t.TempDir() + "/bad_types.db"
	_, err := engine.OpenURL(context.Background(), dsn, engine.WithTypes(&DocWithSQLInjectionTag{}))
	require.Error(t, err)
	require.ErrorIs(t, err, engine.ErrValidation)
}
...
func TestOpenURL_ContextCanceledDuringRegistration(t *testing.T) {
	...
	_, err := engine.OpenURL(ctx, dsn, engine.WithTypes(&Product{}))
	require.Error(t, err)
...
func TestRegister_RejectsEagerOnNonLinkField(t *testing.T) {
	...
	_, err := engine.OpenURL(context.Background(), dsn, engine.WithTypes(&badEagerType{}))
	require.Error(t, err)
	require.ErrorIs(t, err, engine.ErrValidation)
