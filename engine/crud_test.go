...
func TestInsertWithCustomID(t *testing.T) {
	...
	found, err := engine.FindByID[Product](ctx, db, "custom-123")
	require.NoError(t, err)
...
func TestFindByID_NotFound(t *testing.T) {
	...
	_, err := engine.FindByID[Product](ctx, db, "nonexistent")
...
func TestFindByIDs(t *testing.T) {
	...
	require.NoError(t, engine.SaveAll(ctx, db, []*Product{p1, p2, p3}))

	// Fetch two of three
	docs, err := engine.FindByIDs[Product](ctx, db, []string{p1.ID, p3.ID})
	require.NoError(t, err)
	assert.Len(t, docs, 2)
...
func TestFindByIDs_MissingIDs(t *testing.T) {
	...
	docs, err := engine.FindByIDs[Product](ctx, db, []string{p.ID, "nonexistent"})
	require.NoError(t, err)
	assert.Len(t, docs, 1)
...
func TestFindByIDs_Empty(t *testing.T) {
	...
	docs, err := engine.FindByIDs[Product](ctx, db, nil)
	require.NoError(t, err)
	assert.Empty(t, docs)
}
...
func TestQuerySet_Update_BulkPriceUpdate(t *testing.T) {
	...
	require.NoError(t, engine.SaveAll(ctx, db, []*Product{
		{Name: "A", Price: 1.0},
		{Name: "B", Price: 2.0},
		{Name: "C", Price: 99.0},
	}))

	count, err := engine.NewQuery[Product](db, where.Field("price").Lt(10.0)).
		Update(ctx, engine.SetFields{"price": 50.0})
	require.NoError(t, err)
	assert.Equal(t, int64(2), count)

	updated, err := engine.NewQuery[Product](db, where.Field("price").Eq(50.0)).All(ctx)
	require.NoError(t, err)
...
func TestSave_UpdateWhenIDPresent(t *testing.T) {
	...
	found, err := engine.FindByID[Product](ctx, db, originalID)
	require.NoError(t, err)
...
func TestDelete(t *testing.T) {
	...
	_, err := engine.FindByID[Product](ctx, db, p.ID)
	require.ErrorIs(t, err, engine.ErrNotFound)
}
...
func TestRefresh(t *testing.T) {
	...
	require.NoError(t, engine.Refresh(ctx, db, p))
...
func TestUniqueConstraint_UpdateKeepsSameValue(t *testing.T) {
	...
	found, err := engine.FindByID[UniqueProduct](ctx, db, p.ID)
	require.NoError(t, err)
...
func TestNullableUnique_MultipleNils(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, u1))

	u2 := &NullableUniqueUser{Username: "bob"}
	require.NoError(t, engine.Save(ctx, db, u2))
}
...
func TestInsertMany(t *testing.T) {
	...
	require.NoError(t, engine.SaveAll(ctx, db, products))

	for _, p := range products {
		assert.NotEmpty(t, p.ID)
	}

	all, err := engine.NewQuery[Product](db).All(ctx)
	require.NoError(t, err)
	assert.Len(t, all, 3)
}
...
func TestDeleteMany(t *testing.T) {
	...
	count, err := engine.NewQuery[Product](db, where.Field("price").Gt(10.0)).Delete(ctx)
	require.NoError(t, err)
	assert.Equal(t, int64(2), count)

	remaining, err := engine.NewQuery[Product](db).All(ctx)
	require.NoError(t, err)
...
func TestFindOneAndUpdate(t *testing.T) {
	...
	updated, err := engine.NewQuery[Product](db, where.Field("name").Eq("Widget")).UpdateOne(ctx, engine.SetFields{"price": 99.0})
	require.NoError(t, err)
...
func TestFindOneAndUpdate_NotFound(t *testing.T) {
	...
	_, err := engine.NewQuery[Product](db, where.Field("name").Eq("Nonexistent")).UpdateOne(ctx, engine.SetFields{"price": 99.0})
	require.ErrorIs(t, err, engine.ErrNotFound)
}
...
func TestFindOneAndUpdate_FieldNotFound(t *testing.T) {
	...
	_, err := engine.NewQuery[Product](db, where.Field("name").Eq("Widget")).UpdateOne(ctx, engine.SetFields{"nonexistent": "x"})
	require.Error(t, err)
...
func TestFindOneAndUpdate_FieldValidatedBeforeLookup(t *testing.T) {
	...
	_, err := engine.NewQuery[Product](db, where.Field("name").Eq("absent")).UpdateOne(ctx, engine.SetFields{"nonexistent": "x"})
	require.Error(t, err)
...
func TestFindOneAndUpdate_MultipleMatches(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, &Product{Name: "Widget", Price: 10.0}))
	require.NoError(t, engine.Save(ctx, db, &Product{Name: "Widget", Price: 20.0}))

	_, err := engine.NewQuery[Product](db, where.Field("name").Eq("Widget")).UpdateOne(ctx, engine.SetFields{"price": 99.0})
	require.ErrorIs(t, err, engine.ErrMultipleMatches)

	all, err := engine.NewQuery[Product](db).Sort("price", engine.Asc).All(ctx)
	require.NoError(t, err)
	require.Len(t, all, 2)
...
func TestFindOrCreate_Insert(t *testing.T) {
	...
	doc, inserted, err := engine.NewQuery[Product](db, where.Field("name").Eq("Widget")).GetOrCreate(ctx, &Product{Name: "Widget", Price: 1.0})
	require.NoError(t, err)
...
func TestFindOrCreate_Existing(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, existing))

	doc, inserted, err := engine.NewQuery[Product](db, where.Field("name").Eq("Widget")).
...
func TestFindOneAndUpsert_Insert(t *testing.T) {
	...
	doc, inserted, err := engine.NewQuery[Product](db, where.Field("name").Eq("Widget")).UpsertOne(ctx, &Product{Name: "Widget", Price: 1.0}, engine.SetFields{"price": 5.0})
	require.NoError(t, err)
	...
	found, err := engine.FindByID[Product](ctx, db, doc.ID)
	require.NoError(t, err)
...
func TestFindOneAndUpsert_Update(t *testing.T) {
	...
	count, err := engine.NewQuery[Product](db).Count(ctx)
	require.NoError(t, err)
...
func TestFindOneAndUpsert_MultipleMatches(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, &Product{Name: "Widget", Price: 1.0}))
	require.NoError(t, engine.Save(ctx, db, &Product{Name: "Widget", Price: 2.0}))

	_, _, err := engine.NewQuery[Product](db, where.Field("name").Eq("Widget")).UpsertOne(ctx, &Product{Name: "Widget"}, engine.SetFields{"price": 99.0})
	require.ErrorIs(t, err, engine.ErrMultipleMatches)
}
...
func TestFindOneAndUpsert_FieldNotFound(t *testing.T) {
	...
	_, _, err := engine.NewQuery[Product](db, where.Field("name").Eq("Widget")).UpsertOne(ctx, &Product{Name: "Widget"}, engine.SetFields{"nonexistent": "x"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "field")
}
...
func TestFindOneAndUpsert_SoftDeletedSkippedByDefault(t *testing.T) {
	...
	doc, inserted, err := engine.NewQuery[SoftProduct](db, where.Field("name").Eq("Widget")).UpsertOne(ctx, &SoftProduct{Name: "Widget", Price: 10.0}, engine.SetFields{"price": 20.0})
	require.NoError(t, err)
...
func TestFindOneAndUpsert_IncludeDeletedUpdates(t *testing.T) {
	...
	doc, inserted, err := engine.NewQuery[SoftProduct](db, where.Field("name").Eq("Widget")).
		IncludeDeleted().
		UpsertOne(ctx, &SoftProduct{Name: "Widget", Price: 10.0}, engine.SetFields{"price": 20.0})
	require.NoError(t, err)
...
func TestFindOneAndUpsert_HookOrder_InsertPath(t *testing.T) {
	...
	_, inserted, err := engine.NewQuery[orderingDoc](db, where.Field("name").Eq("Widget")).UpsertOne(ctx, &orderingDoc{Name: "Widget"}, engine.SetFields{})
	require.NoError(t, err)
	assert.True(t, inserted)
	assert.Equal(t,
...
func TestFindOneAndUpsert_HookOrder_UpdatePath(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, &orderingDoc{Name: "Widget"}))
...
func TestDelete_MissingIDWrapsErrValidation(t *testing.T) {
	...
	err := engine.Delete(ctx, db, &Product{Name: "NoID"})
	require.Error(t, err)
...
func seedEagerHouse(ctx context.Context, t *testing.T, db *engine.DB, name string) *EagerHouse {
	t.Helper()
	...
	h := &EagerHouse{Name: name, Door: engine.NewLink(door), Owner: engine.NewLink(owner)}
...
func TestFindByID_HonorsEagerTag(t *testing.T) {
	...
	got, err := engine.FindByID[EagerHouse](ctx, db, h.ID)
	require.NoError(t, err)
...
func TestFindByID_WithoutFetchLinksSuppressesEager(t *testing.T) {
	...
	got, err := engine.FindByID[EagerHouse](ctx, db, h.ID, engine.WithoutFetchLinks())
	require.NoError(t, err)
...
func TestFindByIDs_HonorsEagerTag(t *testing.T) {
	...
	got, err := engine.FindByIDs[EagerHouse](ctx, db, []string{h1.ID, h2.ID})
	require.NoError(t, err)
	require.Len(t, got, 2)
	for _, h := range got {
...
func TestRefresh_HonorsEagerTag(t *testing.T) {
	...
	require.NoError(t, engine.Refresh(ctx, db, stale))
...
func TestFindOneAndUpdate_HonorsEagerTag(t *testing.T) {
	...
	got, err := engine.NewQuery[EagerHouse](db, where.Field("_id").Eq(h.ID)).UpdateOne(ctx, engine.SetFields{"name": "Renamed"})
	require.NoError(t, err)
...
func TestFindOneAndUpsert_HonorsEagerTag_HitPath(t *testing.T) {
	...
	got, inserted, err := engine.NewQuery[EagerHouse](db, where.Field("_id").Eq(h.ID)).UpsertOne(ctx, &EagerHouse{Name: "should-not-insert"}, engine.SetFields{"name": "Updated"})
	require.NoError(t, err)
	assert.False(t, inserted)
...
func TestFindOneAndUpsert_HonorsEagerTag_MissPath(t *testing.T) {
	...
	defaults := &EagerHouse{Name: "Fresh", Door: engine.NewLink(door)}
	got, inserted, err := engine.NewQuery[EagerHouse](db, where.Field("name").Eq("Fresh")).UpsertOne(ctx, defaults, engine.SetFields{})
	require.NoError(t, err)
	assert.True(t, inserted)
...
func TestFindOrCreate_HonorsEagerTag(t *testing.T) {
	...
	got, inserted, err := engine.NewQuery[EagerHouse](db, where.Field("_id").Eq(h.ID)).GetOrCreate(ctx, &EagerHouse{Name: "should-not-insert"})
	require.NoError(t, err)
	assert.False(t, inserted)
	assert.True(t, got.Door.IsLoaded(),
...
func TestSaveAll_EmptyInput(t *testing.T) {
	db := dentest.MustOpen(t, &Product{})
	require.NoError(t, engine.SaveAll[Product](context.Background(), db, nil))
}
...
func TestSaveAll_MixedInsertAndUpdate(t *testing.T) {
	...
	all, err := engine.NewQuery[Product](db).Sort("name", engine.Asc).All(ctx)
	require.NoError(t, err)
	require.Len(t, all, 2)
...
func TestSaveAll_FailFastRollsBack(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, &uniqueNameDoc{Name: "Bad"}))
	...
	count, err := engine.NewQuery[uniqueNameDoc](db,
...
func TestDeleteAll_EmptyInput(t *testing.T) {
	db := dentest.MustOpen(t, &Product{})
	require.NoError(t, engine.DeleteAll[Product](context.Background(), db, nil))
}
...
func TestDeleteAll_BatchOfDocs(t *testing.T) {
	...
	require.NoError(t, engine.DeleteAll(ctx, db, docs[:2])) // delete A and B
...
func TestRefreshAll_EmptyInput(t *testing.T) {
	db := dentest.MustOpen(t, &Product{})
	require.NoError(t, engine.RefreshAll[Product](context.Background(), db, nil))
}
...
func TestRefreshAll_PicksUpExternalChanges(t *testing.T) {
	...
	_, err := engine.NewQuery[Product](db).Update(ctx, engine.SetFields{"price": 99.0})
	require.NoError(t, err)
	...
	require.NoError(t, engine.RefreshAll(ctx, db, docs))
