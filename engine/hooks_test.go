...
func TestBeforeSave_Hook(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, a))

	assert.Equal(t, "slug-Hello", a.Slug)
	assert.Equal(t, 5, a.WordCount)

	found, err := engine.FindByID[Article](ctx, db, a.ID)
...
func TestStructTagValidation_RejectsInvalidUpdate(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, v))
...
func TestBeforeInsert_BlocksInsert(t *testing.T) {
	...
	_, err = engine.FindByID[FailBeforeDoc](ctx, db, d.ID)
...
func TestDeleteHooks(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, d))
	require.NoError(t, engine.Delete(ctx, db, d))
...
func TestBeforeSave_ErrorOnUpdate(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, d)) // count=1, passes

	d.Name = "Updated"
	err := engine.Save(ctx, db, d) // count=2, fails
	require.Error(t, err)
