...
func runOnBothBackends(t *testing.T, doc document.Document, body func(*testing.T, *engine.DB)) {
	t.Helper()
	for name, openDB := range map[string]func(*testing.T) *engine.DB{
		"sqlite":   func(t *testing.T) *engine.DB { return dentest.MustOpen(t, doc) },
		"postgres": func(t *testing.T) *engine.DB { return dentest.MustOpenPostgresDefault(t, doc) },
	} {
...
func TestNestedField_UniqueRejectsDuplicate(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, first))

	second := &nestedUser{Email: "b@example.com", Profile: nestedUserProfile{Slug: "ada"}}
	err := engine.Save(ctx, db, second)
	require.Error(t, err, "duplicate nested Profile.Slug must be rejected")
	assert.ErrorIs(t, err, engine.ErrDuplicate)
}

func TestNestedField_IndexIsRegistered(t *testing.T) {
	db := dentest.MustOpen(t, &nestedUser{})

	meta, err := engine.Meta[nestedUser](db)
	require.NoError(t, err)

	byName := make(map[string]engine.IndexDefinition, len(meta.Indexes))
...
func TestNestedField_NilPointerSurvivesRegisterAndCRUD(t *testing.T) {
	...
	found, err := engine.FindByID[nestedUserPointer](ctx, db, doc.ID)
	require.NoError(t, err)
	...
	require.NoError(t, engine.Save(ctx, db, other))
}
...
func TestNestedField_TwoLevelsDeepIndex(t *testing.T) {
	...
	require.NoError(t, engine.SaveAll(ctx, db, []*nestedDeepDoc{
	...
	results, err := engine.NewQuery[nestedDeepDoc](db, where.Field("addr.city.zip").Eq("10115")).All(ctx)
	require.NoError(t, err)
	assert.Len(t, results, 2)

	meta, err := engine.Meta[nestedDeepDoc](db)
	require.NoError(t, err)
	var found bool
	for _, idx := range meta.Indexes {
...
func TestNestedField_SetFieldsOnValueStructWorks(t *testing.T) {
	...
	updated, err := engine.NewQuery[nestedUser](db, where.Field("_id").Eq(doc.ID)).
		UpdateOne(ctx, engine.SetFields{"profile.bio": "new"})
	require.NoError(t, err)
...
func TestNestedField_SetFieldsThroughNilPointerErrors(t *testing.T) {
	...
	_, err := engine.NewQuery[nestedUserPointer](db, where.Field("_id").Eq(doc.ID)).
		UpdateOne(ctx, engine.SetFields{"profile.bio": "new"})
...
func TestNestedFieldParity_UniqueRejectsDuplicate(t *testing.T) {
	runOnBothBackends(t, &nestedUser{}, func(t *testing.T, db *engine.DB) {
		ctx := context.Background()
...
func TestNestedField_CompositeUniqueTogetherMetaSpansLevels(t *testing.T) {
	...
	meta, err := engine.Meta[compositeNestedDoc](db)
	require.NoError(t, err)
...
func TestNestedFieldParity_CompositeUniqueTogetherRejectsCollision(t *testing.T) {
	runOnBothBackends(t, &compositeNestedDoc{}, func(t *testing.T, db *engine.DB) {
		ctx := context.Background()
...
func TestNestedFieldParity_FTSCompositeFlatAndNested(t *testing.T) {
	runOnBothBackends(t, &nestedFTSMixedDoc{}, func(t *testing.T, db *engine.DB) {
		ctx := context.Background()

		require.NoError(t, engine.SaveAll(ctx, db, []*nestedFTSMixedDoc{
		...
		hits, err := engine.NewQuery[nestedFTSMixedDoc](db).Search(ctx, "engine")
		require.NoError(t, err)
		require.Len(t, hits, 1)
		assert.Equal(t, "ada", hits[0].Profile.Slug)

		// Match in the nested field.
		hits, err = engine.NewQuery[nestedFTSMixedDoc](db).Search(ctx, "recursion")
		require.NoError(t, err)
		require.Len(t, hits, 1)
		...
		hits, err = engine.NewQuery[nestedFTSMixedDoc](db).Search(ctx, "alonzo")
		require.NoError(t, err)
...
func TestNestedFieldParity_FTSOnNilPointerSurvives(t *testing.T) {
	runOnBothBackends(t, &nestedFTSPtrDoc{}, func(t *testing.T, db *engine.DB) {
		ctx := context.Background()
		...
		require.NoError(t, engine.Save(ctx, db, &nestedFTSPtrDoc{Email: "a@example.com"}))

		// A non-nil sibling so the FTS infrastructure has at least one
		// indexed row to scan.
		require.NoError(t, engine.Save(ctx, db, &nestedFTSPtrDoc{
			Email:   "b@example.com",
		...
		hits, err := engine.NewQuery[nestedFTSPtrDoc](db).Search(ctx, "lovelace")
		require.NoError(t, err)
		require.Len(t, hits, 1)
...
func TestNestedFieldParity_FTSOnNestedField(t *testing.T) {
	runOnBothBackends(t, &nestedFTSUser{}, func(t *testing.T, db *engine.DB) {
		ctx := context.Background()

		require.NoError(t, engine.SaveAll(ctx, db, []*nestedFTSUser{
		...
		results, err := engine.NewQuery[nestedFTSUser](db).Search(ctx, "lovelace")
		require.NoError(t, err)
		require.Len(t, results, 1)
		assert.Equal(t, "ada", results[0].Profile.Slug)

		results, err = engine.NewQuery[nestedFTSUser](db).Search(ctx, "compilers")
		require.NoError(t, err)
		require.Len(t, results, 1)
...
func TestNestedFieldParity_ProjectNestedPath(t *testing.T) {
	runOnBothBackends(t, &nestedUser{}, func(t *testing.T, db *engine.DB) {
		...
		require.NoError(t, engine.SaveAll(ctx, db, []*nestedUser{
		...
		require.NoError(t, engine.NewQuery[nestedUser](db).Sort("email", engine.Asc).Project(ctx, &out))
		require.Len(t, out, 2)
