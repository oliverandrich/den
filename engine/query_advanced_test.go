...
func seedEvents(t *testing.T, db *engine.DB) {
	t.Helper()
	ctx := context.Background()
	require.NoError(t, engine.SaveAll(ctx, db, []*Event{
...
func TestHasKey(t *testing.T) {
	...
	require.NoError(t, engine.SaveAll(ctx, db, []*DocWithMap{
	...
	results, err := engine.NewQuery[DocWithMap](db, where.Field("metadata").HasKey("color")).All(ctx)
...
func TestHasKey_NoMatch(t *testing.T) {
	...
	require.NoError(t, engine.Save(ctx, db, &DocWithMap{
...
func TestMultiSort(t *testing.T) {
	...
	results, err := engine.NewQuery[Event](db).
		Sort("category", engine.Asc).
		Sort("priority", engine.Desc).
		All(ctx)
	require.NoError(t, err)
...
func TestRegExp(t *testing.T) {
	...
	results, err := engine.NewQuery[Event](db, where.Field("name").RegExp(".*Meeting.*")).All(ctx)
	require.NoError(t, err)
...
func TestRegExp_Compiled(t *testing.T) {
	...
	results, err := engine.NewQuery[Event](db, where.Field("name").RegExp(re)).All(ctx)
	require.NoError(t, err)
	assert.Len(t, results, 2)
}
...
func TestRegExp_NoMatch(t *testing.T) {
	...
	results, err := engine.NewQuery[Event](db, where.Field("name").RegExp("^Nonexistent")).All(ctx)
	require.NoError(t, err)
	assert.Empty(t, results)
}
...
func TestFieldRef(t *testing.T) {
	...
	results, err := engine.NewQuery[Event](db, where.Field("end").Gt(where.FieldRef("start"))).All(ctx)
	require.NoError(t, err)
...
func TestFieldRef_Eq(t *testing.T) {
	...
	results, err := engine.NewQuery[Event](db, where.Field("end").Eq(where.FieldRef("start"))).All(ctx)
	require.NoError(t, err)
	require.Len(t, results, 1)
