...
func (p RevProduct) DenSettings() engine.Settings {
...
func (p SoftRevProduct) DenSettings() engine.Settings {
	return engine.Settings{UseRevision: true}
}
...
func TestRevision_Conflict(t *testing.T) {
	...
	p2, err := engine.FindByID[RevProduct](ctx, db, p.ID)
...
func TestRevision_EmptyRevAfterLoad_Conflicts(t *testing.T) {
	...
	loaded, err := engine.FindByID[RevProduct](ctx, db, original.ID)
	require.NoError(t, err)
	loaded.Rev = "" // manually clear after load
	loaded.Price = 99.0

	err = engine.Save(ctx, db, loaded)
	require.ErrorIs(t, err, engine.ErrRevisionConflict,
...
func TestRevision_ConcurrentUpdates_ExactlyOneSucceeds(t *testing.T) {
	...
	for i := range N {
		wg.Go(func() {
			...
			errs <- engine.Save(ctx, db, docs[i])
		})
	}
	close(start)
	wg.Wait()
	...
	for err := range errs {
		...
		case err == nil:
			success++
		case errors.Is(err, engine.ErrRevisionConflict):
			conflicts++
		default:
...
func TestRevision_SoftDeleteRestoreUpdate(t *testing.T) {
	...
	require.NoError(t, engine.Delete(ctx, db, p)) // soft-delete
...
func TestRevision_ConcurrentSoftDeleteUpdate(t *testing.T) {
	...
	a, err := engine.FindByID[SoftRevProduct](ctx, db, p.ID)
	require.NoError(t, err)
	b, err := engine.FindByID[SoftRevProduct](ctx, db, p.ID)
	require.NoError(t, err)
	...
	found, err := engine.FindByID[SoftRevProduct](ctx, db, p.ID)
	require.NoError(t, err)
...
type SoftRevHouse struct {
	document.Base
	Name string                      `json:"name"`
	Door engine.Link[SoftRevProduct] `json:"door"`
}
...
func TestRevision_CascadeSoftDeleteBumpsLinkedRevision(t *testing.T) {
	...
	house := &SoftRevHouse{Name: "Home", Door: engine.NewLink(door)}
	...
	found, err := engine.NewQuery[SoftRevProduct](db).IncludeDeleted().First(ctx)
	require.NoError(t, err)
	assert.True(t, found.IsDeleted(), "DeletedAt must survive the stale Update attempt")
	assert.Equal(t, "v1", found.Name, "stale Update must not take effect")
}
...
func TestRevision_SoftDeleteIgnoreRevision(t *testing.T) {
	...
	p.Rev = "stale"
	require.NoError(t, engine.Delete(ctx, db, p, engine.IgnoreRevision()))
...
func TestRevision_IgnoreRevision(t *testing.T) {
	...
	p2, _ := engine.FindByID[RevProduct](ctx, db, p.ID)
	p2.Price = 30.0
	require.NoError(t, engine.Save(ctx, db, p2))

	// Force write with stale revision
	p.Price = 99.0
	require.NoError(t, engine.Save(ctx, db, p, engine.IgnoreRevision()))

	found, err := engine.FindByID[RevProduct](ctx, db, p.ID)
	require.NoError(t, err)
