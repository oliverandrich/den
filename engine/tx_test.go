...
func TestRunInTransaction_Commit(t *testing.T) {
	...
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		found, err := engine.FindByID[Product](ctx, tx, p.ID)
		if err != nil {
			return err
		}
		found.Price = 99.0
		return engine.Save(ctx, tx, found)
	})
...
func TestRunInTransaction_Rollback(t *testing.T) {
	...
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		...
		found.Price = 99.0
		if err := engine.Save(ctx, tx, found); err != nil {
			return err
		}
...
func TestTxInsert(t *testing.T) {
	...
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		...
		if err := engine.Save(ctx, tx, p); err != nil {
			return err
		}
		insertedID = p.ID
		return nil
	...
	found, err := engine.FindByID[Product](ctx, db, insertedID)
	require.NoError(t, err)
...
func TestDeleteMany_SoftDelete(t *testing.T) {
	...
	count, err := engine.NewQuery[SoftProduct](db, where.Field("price").Gt(10.0)).Delete(ctx)
	require.NoError(t, err)
	assert.Equal(t, int64(2), count)

	// Soft-deleted should be hidden from normal queries
	remaining, err := engine.NewQuery[SoftProduct](db).All(ctx)
	require.NoError(t, err)
	assert.Len(t, remaining, 1)
	assert.Equal(t, "Keep", remaining[0].Name)

	// But still accessible with IncludeDeleted
	all, err := engine.NewQuery[SoftProduct](db).IncludeDeleted().All(ctx)
	require.NoError(t, err)
	assert.Len(t, all, 3)
}
...
func TestRunInTransaction_PanicRecovery(t *testing.T) {
	...
	assert.Panics(t, func() {
		_ = engine.RunInTransaction(ctx, db, func(_ *engine.Tx) error {
...
func TestTxFindByID_NotFound(t *testing.T) {
	...
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		_, err := engine.FindByID[Product](ctx, tx, "nonexistent")
		return err
	})
	require.ErrorIs(t, err, engine.ErrNotFound)
}
...
func TestInsertMany_Rollback(t *testing.T) {
	...
	err := engine.SaveAll(ctx, db, products)
	require.Error(t, err)

	// No documents should persist after transaction rollback
	all, err := engine.NewQuery[FailBeforeDoc](db).All(ctx)
	require.NoError(t, err)
...
func TestTxLockByID(t *testing.T) {
	...
	for name, db := range dbs {
		t.Run(name, func(t *testing.T) {
			...
			err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
				locked, err := engine.LockByID[Product](ctx, tx, p.ID)
				if err != nil {
					return err
				}
...
func TestTxLockByID_NotFound(t *testing.T) {
	...
	for name, db := range dbs {
		t.Run(name, func(t *testing.T) {
			ctx := context.Background()
			err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
				_, err := engine.LockByID[Product](ctx, tx, "missing-id")
				return err
			})
...
func TestTxLockByID_SerializesConcurrentWriters(t *testing.T) {
	...
	assertSerializesUnderContention(t, db, func(ctx context.Context, tx *engine.Tx) error {
		_, err := engine.LockByID[Product](ctx, tx, p.ID)
		return err
	})
}
...
func runContendedTx(t *testing.T, db *engine.DB, id string) (locked <-chan struct{}, release func()) {
	t.Helper()
	...
	go func() {
		...
		ctx := context.Background()
		err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
			if _, err := engine.LockByID[Product](ctx, tx, id); err != nil {
				return err
			}
			close(lockedCh)
			<-releaseCh
...
func TestTxLockByID_SkipLocked_ReturnsNotFoundOnContention(t *testing.T) {
	...
	start := time.Now()
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		_, err := engine.LockByID[Product](ctx, tx, p.ID, engine.SkipLocked())
		return err
	})
...
func TestTxLockByID_NoWait_ReturnsErrLockedOnContention(t *testing.T) {
	...
	start := time.Now()
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		_, err := engine.LockByID[Product](ctx, tx, p.ID, engine.NoWait())
		return err
	})
	elapsed := time.Since(start)
...
func TestTxLockByID_Options_SQLiteNoop(t *testing.T) {
	...
	for name, opt := range map[string]engine.LockOption{
		"SkipLocked": engine.SkipLocked(),
		"NoWait":     engine.NoWait(),
	} {
		t.Run(name, func(t *testing.T) {
			err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
				locked, err := engine.LockByID[Product](ctx, tx, p.ID, opt)
				if err != nil {
					return err
				}
				assert.Equal(t, "Widget", locked.Name)
...
func TestTxLockByID_ConflictingOptions_Rejected(t *testing.T) {
	...
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		_, err := engine.LockByID[Product](ctx, tx, p.ID, engine.SkipLocked(), engine.NoWait())
		return err
	})
	require.Error(t, err)
	...
	err = engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		_, err := engine.LockByID[Product](ctx, tx, p.ID, engine.NoWait(), engine.SkipLocked())
		return err
	})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "mutually exclusive")
...
func TestLockByID_NGoroutines_ExactlyOneHolder(t *testing.T) {
	...
	for range N {
		wg.Go(func() {
			err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
				if _, err := engine.LockByID[Product](ctx, tx, p.ID); err != nil {
					return err
				}
...
func assertSerializesUnderContention(t *testing.T, db *engine.DB, acquire func(ctx context.Context, tx *engine.Tx) error) {
	t.Helper()
	ctx := context.Background()
...
func TestAdvisoryLock_SameKeySerializes(t *testing.T) {
	db := dentest.MustOpenPostgres(t, dentest.PostgresURL(), &Product{})
	assertSerializesUnderContention(t, db, func(ctx context.Context, tx *engine.Tx) error {
		return engine.AdvisoryLock(ctx, tx, advisoryKeySerialize)
	})
}
...
func TestAdvisoryLock_DifferentKeysParallel(t *testing.T) {
	...
	run := func(wg *sync.WaitGroup, key int64) {
		wg.Go(func() {
			err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
				if err := engine.AdvisoryLock(ctx, tx, key); err != nil {
					return err
				}
				time.Sleep(hold)
				return nil
...
func TestAdvisoryLock_ReleasedOnRollback(t *testing.T) {
	...
	wg.Go(func() {
		...
		start := time.Now()
		err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
			return engine.AdvisoryLock(ctx, tx, advisoryKeyRollback)
		})
		elapsed := time.Since(start)
	...
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		if err := engine.AdvisoryLock(ctx, tx, advisoryKeyRollback); err != nil {
			return err
		}
		close(tx1Released)
...
func TestAdvisoryLock_SQLiteNoop(t *testing.T) {
	...
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		return engine.AdvisoryLock(ctx, tx, 42)
	})
	require.NoError(t, err,
...
func TestScope_CRUDWorksWithBothDBAndTx(t *testing.T) {
	run := func(t *testing.T, ctx context.Context, s engine.Scope) {
		t.Helper()
		p := &Product{Name: "Scope", Price: 12.5}
		require.NoError(t, engine.Save(ctx, s, p))
		require.NotEmpty(t, p.ID)

		got, err := engine.FindByID[Product](ctx, s, p.ID)
		require.NoError(t, err)
		assert.Equal(t, "Scope", got.Name)

		got.Price = 20
		require.NoError(t, engine.Save(ctx, s, got))

		refreshed, err := engine.FindByID[Product](ctx, s, p.ID)
		require.NoError(t, err)
		assert.InDelta(t, 20.0, refreshed.Price, 0.001)

		require.NoError(t, engine.Delete(ctx, s, refreshed))
		_, err = engine.FindByID[Product](ctx, s, p.ID)
		require.ErrorIs(t, err, engine.ErrNotFound)
	}
