...
func TestNewTxQuery_All(t *testing.T) {
	...
	for name, db := range dbs {
		t.Run(name, func(t *testing.T) {
			...
			require.NoError(t, engine.Save(ctx, db, &Product{Name: "A", Price: 1}))
			require.NoError(t, engine.Save(ctx, db, &Product{Name: "B", Price: 2}))
			require.NoError(t, engine.Save(ctx, db, &Product{Name: "C", Price: 3}))

			err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
				items, err := engine.NewQuery[Product](tx).
...
func TestNewTxQuery_First(t *testing.T) {
	...
	ctx := context.Background()
	require.NoError(t, engine.Save(ctx, db, &Product{Name: "Only"}))

	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		found, err := engine.NewQuery[Product](tx).First(ctx)
		if err != nil {
			return err
		}
...
func TestNewTxQuery_First_NotFound(t *testing.T) {
	...
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		_, err := engine.NewQuery[Product](tx).First(ctx)
		return err
	})
...
func TestNewTxQuery_ForUpdate_NoWait(t *testing.T) {
	...
	start := time.Now()
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		_, err := engine.NewQuery[Product](tx).
			ForUpdate(engine.NoWait()).
			All(ctx)
		return err
	})
	elapsed := time.Since(start)
...
func TestNewTxQuery_ForUpdate_ConflictingOptions(t *testing.T) {
	db := dentest.MustOpen(t, &Product{})
	ctx := context.Background()
	require.NoError(t, engine.Save(ctx, db, &Product{Name: "A"}))
	...
	err := engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		_, err := engine.NewQuery[Product](tx).
			ForUpdate(engine.SkipLocked(), engine.NoWait()).
			All(ctx)
		return err
	})
	require.Error(t, err)
	...
	err = engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
		_, err := engine.NewQuery[Product](tx).
			ForUpdate(engine.NoWait(), engine.SkipLocked()).
			First(ctx)
		return err
	})
	require.Error(t, err)
...
func TestNewTxQuery_ForUpdate_OverlappingRowsNoDeadlock(t *testing.T) {
	...
	const N = 20
	for i := range N {
		require.NoError(t, engine.Save(ctx, db, &Product{
			Name:  "row",
			Price: float64(i),
		}))
	}
	...
	go func() {
		defer wg.Done()
		<-startA
		errA = engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
			_, err := engine.NewQuery[Product](tx).
	...
	go func() {
		defer wg.Done()
		<-startB
		errB = engine.RunInTransaction(ctx, db, func(tx *engine.Tx) error {
			_, err := engine.NewQuery[Product](tx).
	...
	require.NotErrorIs(t, errA, engine.ErrDeadlock)
	require.NotErrorIs(t, errB, engine.ErrDeadlock)
}
...
func TestNewTxQuery_ForUpdate_SQLiteNoop(t *testing.T) {
	...
	ctx := context.Background()
	require.NoError(t, engine.Save(ctx, db, &Product{Name: "A"}))
	require.NoError(t, engine.Save(ctx, db, &Product{Name: "B"}))
...
func TestForUpdate_RequiresTransaction(t *testing.T) {
	...
	_, err := engine.NewQuery[Product](db).ForUpdate().All(ctx)
	require.ErrorIs(t, err, engine.ErrLockRequiresTransaction)

	_, err = engine.NewQuery[Product](db).ForUpdate(engine.SkipLocked()).First(ctx)
	require.ErrorIs(t, err, engine.ErrLockRequiresTransaction)
	...
	_, err = engine.NewQuery[Product](db).ForUpdate().Count(ctx)
	require.ErrorIs(t, err, engine.ErrLockRequiresTransaction)
}
