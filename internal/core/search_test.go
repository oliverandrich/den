...
func TestSearch_SQLite(t *testing.T) {
	...
	require.NoError(t, core.SaveAll(ctx, db, []*FTSArticle{
	...
	results, err := core.NewQuery[FTSArticle](db).Search(ctx, "Go")
	...
	for name, db := range dbs {
		t.Run(name, func(t *testing.T) {
			...
			err := core.RunInTransaction(ctx, db, func(tx *core.Tx) error {
				if err := core.Save(ctx, tx, &FTSArticle{
					Title: "TxLocalSecret", Body: "tx-local body", Category: "tech",
				}); err != nil {
					return err
				}
				hits, err := core.NewQuery[FTSArticle](tx).Search(ctx, "TxLocalSecret")
				if err != nil {
					return err
				}
				insideHits = hits
			...
			after, err := core.NewQuery[FTSArticle](db).Search(ctx, "TxLocalSecret")
			require.NoError(t, err)
	...
	for name, db := range dbs {
		t.Run(name, func(t *testing.T) {
			...
			err := core.RunInTransaction(ctx, db, func(tx *core.Tx) error {
				...
				hits, err := core.NewQuery[FTSArticle](tx).Search(ctx, "ShouldNeverReachDisk")
				if err != nil {
					return err
				}
			...
			after, err := core.NewQuery[FTSArticle](db).Search(ctx, "ShouldNeverReachDisk")
			require.NoError(t, err)
...
func TestSearch_SQLite_WithFilter(t *testing.T) {
	...
	results, err := core.NewQuery[FTSArticle](db, where.Field("category").Eq("tech")).Search(ctx, "Go")
	require.NoError(t, err)
	require.Len(t, results, 1)
...
func TestSearch_SQLite_WithLimit(t *testing.T) {
	...
	results, err := core.NewQuery[FTSArticle](db).Limit(2).Search(ctx, "Go")
	require.NoError(t, err)
	assert.Len(t, results, 2)
}
...
func TestSearch_SQLite_NoResults(t *testing.T) {
	...
	require.NoError(t, core.Save(ctx, db, &FTSArticle{Title: "Hello", Body: "World"}))

	results, err := core.NewQuery[FTSArticle](db).Search(ctx, "nonexistent")
	require.NoError(t, err)
...
func TestSearch_Postgres(t *testing.T) {
	...
	results, err := core.NewQuery[FTSArticle](db).Search(ctx, "programming")
	require.NoError(t, err)
	require.Len(t, results, 1)
	assert.Equal(t, "Go Programming", results[0].Title)
}
...
func TestSearch_HonorsAfterCursor(t *testing.T) {
	...
	for name, db := range dbs {
		t.Run(name, func(t *testing.T) {
			...
			first, err := core.NewQuery[FTSArticle](db).Sort("_id", core.Asc).Search(ctx, "Go")
			require.NoError(t, err)
			require.Len(t, first, 3)

			rest, err := core.NewQuery[FTSArticle](db).Sort("_id", core.Asc).After(first[0].ID).Search(ctx, "Go")
			require.NoError(t, err)
