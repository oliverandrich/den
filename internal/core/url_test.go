...
func stubOpener(scheme string) func(ctx context.Context, dsn string) (core.Backend, error) {
	return func(_ context.Context, _ string) (core.Backend, error) {
...
func TestRegisterBackend_ConcurrentDistinctSchemes(t *testing.T) {
	...
	for i := range N {
		...
		wg.Go(func() {
			core.RegisterBackend(scheme, stubOpener(scheme))
		})
	}
	wg.Wait()
	...
	for i := range N {
		scheme := fmt.Sprintf("%s_%d", base, i)
		_, err := core.OpenURL(ctx, scheme+"://x")
...
func TestOpenURL_EmptyDSN(t *testing.T) {
	_, err := core.OpenURL(context.Background(), "")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "empty database URL")
}

func TestOpenURL_MissingScheme(t *testing.T) {
	_, err := core.OpenURL(context.Background(), "no-separator-here")
	require.Error(t, err)
...
func TestOpenURL_UnregisteredScheme(t *testing.T) {
	_, err := core.OpenURL(context.Background(), "nosuch_scheme_"+regTestPrefix(t)+"://x")
	require.Error(t, err)
	require.ErrorIs(t, err, core.ErrUnsupportedScheme,
...
func TestRegisterBackend_PanicsOnEmptyScheme(t *testing.T) {
	require.PanicsWithValue(t, "den: RegisterBackend with empty scheme", func() {
		core.RegisterBackend("", stubOpener("ignored"))
	})
}
...
func TestRegisterBackend_PanicsOnNilOpener(t *testing.T) {
	...
	require.PanicsWithValue(t, "den: RegisterBackend with nil opener for scheme "+scheme, func() {
		core.RegisterBackend(scheme, nil)
	})
}
...
func TestRegisterBackend_CaseInsensitiveDuplicatePanics(t *testing.T) {
	...
	core.RegisterBackend(base, stubOpener(strings.ToLower(base)))

	require.PanicsWithValue(t, "den: duplicate registration for scheme "+strings.ToLower(base), func() {
		core.RegisterBackend(strings.ToUpper(base), stubOpener(strings.ToUpper(base)))
	})
}

func TestRegisterBackend_IsCaseInsensitive(t *testing.T) {
	scheme := regTestPrefix(t) + "_MixedCase"
	core.RegisterBackend(scheme, stubOpener(strings.ToLower(scheme)))
	...
	for _, dsn := range []string{
		...
		_, err := core.OpenURL(context.Background(), dsn)
